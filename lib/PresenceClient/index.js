...
PresenceClient.getUserPresence = (userId, local) =>
  podRequest(
    'GET',
    SymConfigLoader.SymConfig.pathPrefix + `/pod/v3/user/${userId}/presence?local=${local ? 'true' : 'false'}`,
...
PresenceClient.setPresence = status => {
  ...
  return podRequest('POST', SymConfigLoader.SymConfig.pathPrefix + '/pod/v2/user/presence', 'UsersClient/setPresence', body)
}

PresenceClient.registerInterestExtUser = () =>
  podRequest('POST', SymConfigLoader.SymConfig.pathPrefix + '/pod/v1/user/presence/register', 'UsersClient/registerInterestExtUser')
