...
SymBotClient.authenticateExtApp = () => {
  return SymBotAuth.extAppAuthenticate(SymBotClient.config).then(symAuth => {
    return {
      appId: symAuth.appId,
      appToken: symAuth.appToken
    }
  })
}

SymBotClient.authenticateOboApp = () => {
  return SymBotAuth.oboAppAuthenticate(SymBotClient.config).then(symAuth => {
    return symAuth
  })
}

SymBotClient.verifyJwt = (jwt) => {
  return SymBotAuth.verifyJWT(SymBotClient.config, jwt)
}
...
SymBotClient.sendMessageWithMultipleAttachments = MessagesClient.sendMessageWithMultipleAttachments
...
/* SymMessageParser Services */

SymBotClient.getCashtags = SymMessageParser.getCashtags
SymBotClient.getHashtags = SymMessageParser.getHashtags
SymBotClient.getMentions = SymMessageParser.getMentions

/* Other Services */
SymBotClient.formBuilder = FormBuilder
