...
OBOClient.oboGetAllConnections = status =>
  podRequest('get', '/pod/v1/connection/list?status=all', 'OBOClient/oboGetAllConnections')
...
OBOClient.oboGetConnection = userId =>
  podRequest('get', `/pod/v1/connection/user/${userId}/info`, 'OBOClient/oboGetConnection')
...
function send (userToken, conversationId, message, data) {
  ...
  return SymConfigLoader.SymConfig.getAgentHost().then(hostname => {
    const options = {
      ...
      path: PathBuilder.buildAgentPath(`/agent/v4/stream/${conversationId}/message/create`),
      method: 'POST',
      headers: headers,
