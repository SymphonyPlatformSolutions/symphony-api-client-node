...
FirehoseClient.createFirehose = () =>
  agentRequest('post', SymConfigLoader.SymConfig.pathPrefix + '/agent/v4/firehose/create', 'FirehoseClient/createFirehose')
...
FirehoseClient.getEventsFromFirehose = firehoseId => {
  return SymConfigLoader.SymConfig.getAgentHost().then(hostname => {
    const options = {
      ...
      path: SymConfigLoader.SymConfig.pathPrefix + `/agent/v4/firehose/${firehoseId}/read`,
      method: 'GET',
      headers: {
