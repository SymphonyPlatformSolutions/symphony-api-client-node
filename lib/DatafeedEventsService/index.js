...
class DatafeedEventsService extends EventEmitter {
  ...
  _retryConnection(err) {
    SymConfigLoader.SymConfig.rotateAgent().then(() => {
      ...
      if ((err.statusCode === 400 || err.statusCode === 'ECONNRESET' || err.statusCode === 'ECONNREFUSED') && this.readErrors < 3) {
        this.id = null
        this.readErrors++
        this._create()
      } else {
